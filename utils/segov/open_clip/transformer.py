...
def topk_to_one_hot(matrix, k):
    """
    将矩阵每行最大的 k 个值置为 1，其余值置为 0。

    Args:
        matrix (torch.Tensor): 输入矩阵，形状为 (N, M)。
        k (int): 每行选取的最大值数量。

    Returns:
        torch.Tensor: 处理后的矩阵，形状与输入相同。
    """
    # 获取每行前 k 个最大值的索引
    topk_indices = torch.topk(matrix, k, dim=1).indices
...
def expand_matrix_with_identity(matrix):
    """
    将 NxN 的矩阵扩展为 (N+1)x(N+1)，
    添加的一行和一列前 N 个数为 0，第 N+1 个数为 1。

    Args:
        matrix (torch.Tensor): 输入矩阵，形状为 (N, N)。

    Returns:
        torch.Tensor: 扩展后的矩阵，形状为 (N+1, N+1)。
    """
    # 检查输入是否为方阵
    ...
    # 创建一个 (N+1)x(N+1) 的零矩阵
    expanded_matrix = torch.zeros((N + 1, N + 1), dtype=matrix.dtype, device=matrix.device)

    # 将原矩阵拷贝到扩展矩阵的左上角
    expanded_matrix[:N, :N] = matrix

    # 设置右下角为 1
    expanded_matrix[N, N] = 1
    # expanded_matrix[N, :] = 1
    # expanded_matrix[:, N] = 1

    return expanded_matrix

def process_and_compute_distances(img_tensor, patch_size):
    """
    对图像张量进行分块平均处理并计算行之间的距离矩阵。

    Args:
        img_tensor (torch.Tensor): 输入张量，形状为 (1, C, H, W)。
        patch_size (int): 每个 patch 的大小 p。

    Returns:
        torch.Tensor: 成对距离矩阵，形状为 ((H//p) * (W//p), (H//p) * (W//p))。
    """
    # 确保输入形状为 (1, C, H, W)
    assert img_tensor.dim() == 4 and img_tensor.size(0) == 1, "Input must be 4D with shape (1, C, H, W)"
    C, H, W = img_tensor.shape[1:]

    # 检查 H 和 W 是否能被 patch_size 整除
    assert H % patch_size == 0 and W % patch_size == 0, "Height and width must be divisible by patch_size"

    # 1. 对每个 patch 求平均
    pooled = F.avg_pool2d(img_tensor, kernel_size=patch_size, stride=patch_size)
    # 结果形状: (1, C, H//p, W//p)
    ...
    # 3. 展平为 2D 张量: ((H//p) * (W//p), C)
    reshaped = pooled.reshape(-1, C)

    # 4. 使用 pdist 计算成对距离
    distances = F.pdist(reshaped, p=2)

    # 5. 转换为对称距离矩阵
    n = reshaped.size(0)
    dist_matrix = torch.zeros((n, n)).to(distances.device)
    triu_indices = torch.triu_indices(n, n, offset=1)
    dist_matrix[triu_indices[0], triu_indices[1]] = distances
    dist_matrix = dist_matrix + dist_matrix.T.clone()  # 对称化
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, model_type: str = 'ClearCLIP', ignore_residual=True, output_cls_token=False, last_n_layers=1,
                **kwargs):
        if 'am' in kwargs.keys():
            attn_mask = kwargs['am']
            if isinstance(attn_mask, np.ndarray):
                attn_mask = torch.from_numpy(attn_mask).to(x.device)
        else:
            attn_mask = None
        ...
        for idx, blk in enumerate(self.transformer.resblocks[:-last_n_layers]):
            ...
            # if idx >= 1:
            #     x = blk(x, attn_hsi=attn_hsi)
            # else:
            #     x = blk(x)
            # a = x[:196].permute(1, 2, 0).reshape(1, -1, 16, 16)
            # a = x[:784].permute(1, 2, 0).reshape(1, -1, 28, 28)
            # b = apply_pca_on_channels(a)
            # plt.imshow(b[0]);
            # plt.show()
        output = 0
        for blk in self.transformer.resblocks[-last_n_layers:]:
            if ignore_residual:
                output += self.custom_attn(blk.attn, blk.ln_1(x), model_type=model_type, attn_mask=attn_mask)
    ...
    def custom_attn(self, attn_layer, x, model_type='ClearCLIP', attn_mask=None):
        ...
        if model_type == 'vanilla':
            ...
            if attn_mask is not None:
                qq_attn[:, attn_mask == 0] = -torch.inf
                kk_attn[:, attn_mask == 0] = -torch.inf
                vv_attn[:, attn_mask == 0] = -torch.inf
        ...
        attn_output = attn_layer.out_proj(attn_output) # a = attn_weights[0][2, :196].reshape(14, 14)
